...
const float TRAIL_SIZE = 0.75; // 0.0 = all corners move together. 1.0 = max smear (leading corners jump instantly)
const float THRESHOLD_MIN_DISTANCE = 5; // min distance to show trail (units of cursor height)
const float BLUR = 0.5;
const float TRAIL_THICKNESS = 0.35;  // 1.0 = full cursor height, 0.0 = zero height, >1.0 = funky aah
const float TRAIL_THICKNESS_X = 0.25;

const float FADE_ENABLED = 1; // 1.0 to enable fade gradient along the trail, 0.0 to disable
const float FADE_EXPONENT = .75; // exponent for fade gradient along the trail
